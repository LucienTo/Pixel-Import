...
An Aseprite extension that brings any image in as editable pixel art. One
window: a live preview, and the parameters that decide what the result looks
like.

- Detects images that are **already pixel art upscaled** and recovers the
  original pixel for pixel.
- Import several files at once, or a gif, and lay them out as **frames or
  layers**.
- Reduce color with **median cut**, or snap to the palette your sprite already
  uses.
- Nothing touches your sprite until you press Place, and the whole placement is
  a single undo step.

Built and tested against Aseprite 1.3.18.2 (API 41), on Windows.
...
2. In Aseprite, go to **Edit > Preferences > Extensions > Add Extension**, and
   pick the file you downloaded. (On Windows you can usually just double-click
   it instead.)
3. Restart Aseprite.

The command appears at **File > Scripts > Pixel Import: Convert Image**.

Works on Windows, macOS and Linux - it is Lua, and uses no platform-specific
calls. Developed and tested on Windows.

### Updating

Install the newer `.aseprite-extension` the same way. Aseprite replaces the
installed copy when the `version` in `package.json` is higher.

### Uninstall

**Edit > Preferences > Extensions**, select Pixel Import, **Uninstall**.

### Installing from source

For working on it, rather than using it:

```powershell
.\install.ps1
```

That copies the runtime files straight to
`%APPDATA%\Aseprite\extensions\pixelimport`, skipping the packaging step. It is
Windows-only and exists for the edit-reinstall-restart loop. On macOS or Linux,
copy `package.json`, `main.lua` and `src/` into a `pixelimport` folder inside
Aseprite's extensions directory by hand. `tests/` is never needed at runtime.
...
+--------------------------+
|                          |
|      live preview        |   converted result, nearest-neighbor,
|      on a checkerboard    |   on a checkerboard so transparency reads
|                          |
+--------------------------+
[ Import Image ]
Image        [ 1. sprite              v ]
...
Resample     [ Nearest neighbor       v ]

Palette      [ Adaptive (from image)  v ]
             [x] Limit color palette
             [----o----------]  16

Position     [ Snap to center         v ]
Place        [ All images, one per frame (animation) v ]

[ Place on Selected Keyframe ]
```

The window reads top to bottom as the pipeline runs: what came in, how it is
converted, where it goes. The destination controls sit directly above the
button that acts on them rather than being separated from it by every
conversion setting.
...
```powershell
gh release create v0.1.0 pixel-import.aseprite-extension --title "Pixel Import 0.1.0" --notes "First release."
```
